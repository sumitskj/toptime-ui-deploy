    ...
    const fetchBooking = async () => {
      try {
        ...
        const bookingRes = await dispatch(
          getUpcomingUserBooking({ isProfessional: true }),
        ).unwrap();
        if (bookingRes.ok) {
          const bookingJson = await bookingRes
            .json()
            .catch(() => console.log('No professional bookings upcoming'));
          setBooking(bookingJson === undefined ? null : bookingJson);
        }
      } catch (err) {
        console.error('Error in fetching professional upcoming booking ', err);
        setError(true);
      }
      setLoading(false);
    };
...
export default ProfessionalUpcomingBooking;
